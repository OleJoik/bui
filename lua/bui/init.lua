...
function Renderer.new(bufnr, winid)
...
local function create_app_state()
  return {
    first_name = Signal.new("Ada"),
    last_name = Signal.new("Lovelace"),
    email = Signal.new("ada@example.com"),
    city = Signal.new("London"),
    company = Signal.new("Analytical Engines Ltd"),
    focus_index = Signal.new(1),
  }
end

local function App(state)
  return Column({
    Text({
      text = "Profile editor",
    }),
    ...
    Row({
      Input({
        ...
        on_edit = function()
          state.first_name:set(vim.fn.input("First name: ", state.first_name:get()))
        end,
      }),
      Input({
        label = "Last name",
        ...
        on_edit = function()
          state.last_name:set(vim.fn.input("Last name: ", state.last_name:get()))
        end,
      }),
    }, { gap = 2 }),
    ...
    Row({
      Input({
        ...
        on_edit = function()
          state.email:set(vim.fn.input("Email: ", state.email:get()))
        end,
      }),
      Column({
        Input({
          ...
          on_edit = function()
            state.city:set(vim.fn.input("City: ", state.city:get()))
          end,
        }),
        Input({
          label = "Company",
          width = 24,
          value = function()
            return state.company:get()
          end,
          on_edit = function()
            state.company:set(vim.fn.input("Company: ", state.company:get()))
          end,
        }),
      }, { gap = 1 }),
    }, { gap = 2 }),
...
local function clamp_focus(state, renderer)
  local count = renderer:get_focusable_count()
  if count == 0 then
    state.focus_index:set(1)
    return
  end

  local current = state.focus_index:get()
  if current < 1 then
    state.focus_index:set(1)
  elseif current > count then
    state.focus_index:set(count)
  end
end

local function move_focus_direction(direction, state, renderer)
  local count = renderer:get_focusable_count()
  if count == 0 then
    return
  end
  ...
  state.focus_index:set(next_index)
end

local function edit_focused(state, renderer)
  local item = renderer:get_focused_item(state.focus_index:get())
...
function M.open()
  ...
  vim.bo[buf].filetype = "bui"

  local renderer = Renderer.new(buf, win)

  effect(function()
    renderer:render(App(state), state.focus_index:get())
    clamp_focus(state, renderer)
  end)

  local opts = { buffer = buf, silent = true }

  vim.keymap.set("n", "h", function()
    move_focus_direction("left", state, renderer)
  end, opts)

  vim.keymap.set("n", "l", function()
    move_focus_direction("right", state, renderer)
  end, opts)

  vim.keymap.set("n", "k", function()
    move_focus_direction("up", state, renderer)
  end, opts)

  vim.keymap.set("n", "j", function()
    move_focus_direction("down", state, renderer)
  end, opts)

  vim.keymap.set("n", "<CR>", function()
    edit_focused(state, renderer)
  end, opts)
...
function M.setup(opts)
  ...
  if opts.create_command ~= false then
    vim.api.nvim_create_user_command("BuiOpen", function()
      M.open()
    end, { desc = "Open Bui profile editor demo" })
  end
end
